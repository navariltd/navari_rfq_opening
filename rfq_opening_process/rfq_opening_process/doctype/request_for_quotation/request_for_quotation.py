...
def get_committee_members(committee_rec, rfq_name):
	...
	AND
		cm.parent = %(cm)s;""",
        {"cm": committee_rec.committee_name, "rfqname": rfq_name},
        as_dict=1,
    )
...
def make_quote_or_bid_opening_todo(committee, committeemember, rfq):
    ...
    todo.description = (
        "Request for Quotation No: {0} Opening for {1} on: {2} at: {3}".format(
            rfq.name, committeemember, rfq.closing_date, rfq.closing_time
        )
    )
...
def skip_supplier_quotation_opening_by_committee(rfq_name):
    if cint(
        frappe.db.get_single_value(
            "RFQ Opening Settings", "skip_supplier_quotation_opening_by_committee"
        )
    ):
        return True
...
def count_of_received_supplier_quotations_for_supplier(supplier, rfq_name):
    received_sqs_count = frappe.db.sql(
        ...
        {"suppliername": supplier, "rfqname": rfq_name},
    )[0][0]
...
def allow_multiple_supplier_quotations_from_portal(supplier, rfq_name):
    if cint(
        frappe.db.get_single_value(
            "RFQ Opening Settings", "allow_multiple_supplier_quotations_from_portal"
        )
    ):
        return True
...
def get_minimum_expected_quotes_for_rfq(rfq_name):
    ...
    minimum_expected_quotes_for_rfq = frappe.get_value(
        "Request for Quotation", rfq_name, "minimum_expected_quotes"
    )
...
def get_minimum_no_of_members_needed_to_open_sqs(rfq_name):
    ...
    minimum_no_of_members_needed_to_open_sqs = frappe.get_value(
        "Committee", rfq_committee, "minimum_no_of_members_needed_to_open"
    )
...
def send_to_supplier(self):
    ...
    if not skip_supplier_quotation_opening_by_committee(self.name):
        create_quote_or_bid_opening(self)
...
def create_supplier_quotation_override(doc):
    ...
    try:
        ...
        if not allow_multiple_supplier_quotations_from_portal(
            doc.get("supplier"), doc.get("name")
        ):
            frappe.throw(
                _(
                    "Supplier Quotation for Request for Quotation {0} already submitted".format(
                        doc.get("name")
                    )
                )
            )
        ...
        if get_datetime() > get_datetime(closing_datetime):
            frappe.throw(
                _(
                    "Submission Closing Date and Time {0} has already passed".format(
                        closing_datetime
                    )
                )
            )
...
def open_supplier_quotations(rfq_name):
    ...
    minimum_no_of_members_needed_to_open_sqs = (
        get_minimum_no_of_members_needed_to_open_sqs(rfq_name)
    )
    ...
    if received_sqs_for_rfq >= minimum_expected_quotes_for_rfq:
        ...
        if (opened_quote_or_bid_todos + 1) >= minimum_no_of_members_needed_to_open_sqs:
            ...
            frappe.msgprint(
                _("{0} Supplier Quotations opened").format(received_sqs_for_rfq)
            )
    else:
        message = _(
            "The received supplier quote(s) for {0} is {1}, which is less than the expected number of {2} quotes".format(
                rfq_name, received_sqs_for_rfq, minimum_expected_quotes_for_rfq
            )
        )
